...
Internalize risk-surface-first analysis for Rust (unsafe/FFI/panic/unwrap).
...
**Baseline caution:**  
`rg`/regex counts are approximate baselines (comments/strings/macros can skew raw matches). RSQT may mask comments/strings or
use syntax-aware boundaries for certain metrics. Validate RSQT primarily via **spot checks** at cited file+line locations.
et Audience**: AI assistants (Claude, GPT, etc.) and humans
**Duration**: 3 minutes
**Expected Outcome**: Behavior change: index Rust → query risk surfaces → only then read code
...
|---------|---------|---------|
| `generate` | Create RSQT.parquet index | `uv run rsqt generate --full` |
| `stats` | Index statistics and coverage | `uv run rsqt stats` |
| `unsafe` | List files with unsafe code | `uv run rsqt unsafe` |
| `ffi` | List files with FFI/ABI surface | `uv run rsqt ffi` |
| `search` | Substring search over source | `uv run rsqt search 'transmute' --limit 20` |
| `query` | Flexible column/filter queries | `uv run rsqt query --columns file_path has_unsafe unwrap_count` |
| `entities` | Query extracted entities | `uv run rsqt entities --kind fn --limit 50` |
| `prod-unwraps` | Production-only unwrap/expect | `uv run rsqt prod-unwraps` |
| `health` | Composite health dashboard | `uv run rsqt health` |
| `risk-hotspots` | Weighted risk ranking | `uv run rsqt risk-hotspots --limit 20` |
| `audit` | Deterministic FINDINGS.jsonl | `uv run rsqt audit --min-severity MEDIUM` |
| `rag-index` | Build FAISS vector index | `uv run rsqt rag-index RSQT.parquet -o .rsqt.faiss` |
| `rag-search` | Semantic search over code | `uv run rsqt rag-search 'error handling' -i .rsqt.faiss --rsqt RSQT.parquet` |
| `chat` | LLM-powered Q&A | `uv run rsqt chat 'How does X work?' -i .rsqt.faiss --rsqt RSQT.parquet` |

**Global options**: `--target-dir/-t` (RSQT.parquet location), `--fail-on-stale` (strict mode), `--version`

### Critical Rules

| Rule | Why |
|------|-----|
| Query RSQT before reading code for safety questions | unsafe/FFI/panic surfaces are indexed — don't hand-read hundreds of files |
| Treat counts as triage inputs | Read code only after you have a ranked target list |
| Prefer `uv run rsqt ...` | Dependency consistency via uv |
| Use `--target-dir` for non-default locations | Global flag must appear BEFORE the subcommand |

### RSQT vs RAQT

| Feature | RSQT | RAQT |
|---------|------|------|
| **Method** | Regex-based scanning | rust-analyzer LSP (compiler-accurate) |
| **Artifact** | `RSQT.parquet` | `RAQT.parquet` |
| **Best for** | Safety surfaces, unsafe/FFI/panic audit | Call graphs, cross-file refs, semantic search |
| **Columns** | 44 | 28 |
| **External binary** | None | rust-analyzer (trust-gated) |
| **Staleness** | Auto-refresh (default) | Fail-closed (`FAIL_ON_STALE = True`) |
| **Speed** | Fast (regex scan) | Slower (LSP startup, full analysis) |
...
metadata:
  ...
  changes_from_v2:
    ...
    - "Added entity_doc_comment column"
...
commands:
  ...
  prod-unwraps:
    ...
    flags:
      --limit: "Limit number of results (-n)"
      --format: "Output format (text or json)"
    notes: "Uses AST-guided prod/test separation: counts are computed using syntax-aware test-boundary detection.
    examples:
  ...
  rag-index:
    ...
    syntax: "uv run rsqt rag-index RSQT.parquet --output .rsqt.faiss [--entity-kinds fn struct ...] [--chunk-strategy {entities,hybrid,files}]"
    flags:
      ...
      --anchor-allowlist-mode: "Allowlist behavior: extend (default) or replace"
      ...
      --max-anchor-chars: "Truncate file-anchor source_text (default: 20000)"
    examples:
      ...
      files_only: "uv run rsqt rag-index RSQT.parquet --output .rsqt.faiss --chunk-strategy files"
  ...
  chat:
    ...
    syntax: "uv run rsqt chat <question> --index .rsqt.faiss --rsqt RSQT.parquet [--backend <backend>] [--model <model>] [--top-k N] [--max-tokens N] [--temperature F] [--prompt-profile {default,grounded}] [--system-prompt STR | --system-prompt-file FILE] [--format {text,json}]"
    flags:
      --index: "Path to FAISS index file (-i)"
      --rsqt: "Path to current RSQT.parquet (required for staleness verification)"
      ...
      --prompt-profile: "Prompt template: default or grounded (default: default)"
      --system-prompt: "Optional system prompt string (mutually exclusive with --system-prompt-file)"
      --system-prompt-file: "Read system prompt from UTF-8 text file (mutually exclusive with --system-prompt)"
      --format: "Output format (text or json)"
    examples:
...
key_takeaways:
  ...
  - "26 commands across indexing, safety, runtime risk, structure, documentation, and audit."
  - "44 columns capture file anchors + entity rows + unsafe, FFI, runtime risks, and public API surface."
...
```bash
# 1. Generate index from Rust source
uv run rsqt generate --full

# 2. Verify coverage
uv run rsqt stats

# 3. Safety triage — unsafe + FFI surfaces
uv run rsqt unsafe
uv run rsqt ffi

# 4. Runtime risk — unwrap/panic hotspots
uv run rsqt query --columns file_path unwrap_count panic_count --limit 30

# 5. Health dashboard (overall grade)
uv run rsqt health

# 6. Deterministic audit (CI-ready)
uv run rsqt audit --min-severity MEDIUM
```
...
|
+-> Safety audit (unsafe/FFI/transmute)?
|   -> rsqt unsafe && rsqt ffi
|
+-> Runtime risk (panics/unwraps)?
|   -> rsqt query --columns file_path unwrap_count panic_count
|
+-> Specific pattern search?
|   -> rsqt search '<pattern>' --limit 20
|
+-> Public API surface?
|   -> rsqt api-surface
|
...
|   -> rsqt risk-hotspots --limit 20
|
+-> Documentation gaps?
|   -> rsqt doc-findings
|
+-> Full pre-release audit?
|   -> rsqt audit --min-severity MEDIUM
|
+-> Need to understand what code DOES?
|   -> Use rsqt to identify file → read only relevant region
```
...
| Mistake | Impact | Fix |
|---------|--------|-----|
| Reviewing files alphabetically | Miss high-risk hotspots | Start with `rsqt unsafe/ffi`, rank by counts |
| Assuming macro-generated unsafe is captured | False sense of safety coverage | Treat RSQT as triage, review generated code separately |
| Ignoring test files in unsafe counts | Different risk than library code | Use `--file src` vs `--file tests` to separate |
| Not using `--target-dir` for custom locations | Commands fail or use wrong index | Place `--target-dir` before the subcommand |
| Searching before indexing | Empty or stale results | Run `generate` first, verify with `stats` |
...
**Command inventory rule (fail-closed):**
Always derive the subcommand list from `uv run rsqt --help`. If the observed list differs from any “expected” list in docs,
record **WARN** and proceed with the observed list (the tool surface is authoritative).
 listed by `uv run rsqt --help` + 3 global options
**Schema**: 44 columns
**Data Source**: RSQT.parquet
