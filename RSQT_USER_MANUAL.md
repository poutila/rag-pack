...
| `--strict-json` | | Guarantee stdout is strict JSON only (all progress/status to stderr) |
| `--profile` | | Execution profile: `ci` for deterministic output (stable sort, resolved paths, implies `--fail-on-stale`) |
...
| `--prompt-profile` | | Prompt template profile: `default`, `grounded`, or `rust_audit_guru` (default: default) |
...
- `rust_audit_guru`: citation-rich audit mode; produces structured findings with evidence from retrieved chunks.
...
List JSON output schemas for all commands (FR-4 versioned output contract).
...
uv run rsqt schemas --format json
```
...
**Use cases:**
- Discover the JSON output contract for each command
...
**Changes**:
- v3.2.0: Added `schemas` command (FR-4 versioned output contract), `--strict-json` global flag (stdout-only JSON), `--profile ci` global flag (deterministic output), `rust_audit_guru` prompt profile for `chat` command
