...
# Runs all 5 split RSQT general sets sequentially.
...
PARQUET="${PARQUET:-RSQT.parquet}"
INDEX="${INDEX:-.rsqt.faiss}"
QUOTE_BYPASS_MODE="${QUOTE_BYPASS_MODE:-on}"
CACHE_PREFLIGHTS="${CACHE_PREFLIGHTS:-1}"
OUT_ROOT="${OUT_ROOT:-XREF_WORKFLOW_II_new/xref_state/RSQT_GENERAL_5SETS_$(date +%y%m%d_%H%M%S)}"

PACK_DIR="XREF_WORKFLOW_II_new/tools/rag_packs"
PACKS=(
  "pack_rust_audit_rsqt_general_set1_v1_0.yaml"
  "pack_rust_audit_rsqt_general_set2_v1_0.yaml"
  "pack_rust_audit_rsqt_general_set3_v1_0.yaml"
  "pack_rust_audit_rsqt_general_set4_v1_0.yaml"
  "pack_rust_audit_rsqt_general_set5_v1_0.yaml"
)

mkdir -p "${OUT_ROOT}"

echo "== RSQT 5-set batch run =="
echo "repo:        ${REPO_ROOT}"
